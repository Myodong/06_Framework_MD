...
goToListBtn.addEventListener("click", ()=>{
    ...
    // location.pathname    : 현재 요청 주소만을 반환(프로토콜ip,포트 제외)
    // location.search      : 쿼리스트링(?cp=숫자)만 반환

    const pathname =location.pathname; // /board/1/1500

    const queryString = location.search; // ?cp=7

    const url = pathname.substring(0,pathname.lastIndexOf("/"))+queryString; // /board/1?cp=7&key=t&query=테스트
        ...
        $.ajax({
            ...
            success : (result) =>{
                ...
                if(result > 0) { // 성공
                    ...
                    e.target.classList.add('fa-regular'); // 빈하트 클래스 삭제
