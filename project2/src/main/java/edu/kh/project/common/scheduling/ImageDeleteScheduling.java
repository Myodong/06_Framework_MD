...
// IOC(제어의 역전
// -> bean 등록 필요 (@Componentp,@Controller, @Service, @Repository)
@Component
...
//	@Scheduled(cron = "0,15,30,45 * * * * *") // 매 분15초 마다
//	@Scheduled(cron = "0/15 * * * * *") // 0초 부터 시작하여 15초 마다
	@Scheduled(cron = "0 * * * * *") // 매 분 0초 마다
	public void deleteImageFile() {
//		System.out.println("0초마다 출력");
		
		// 1. DB에서 DOARD_IMG 테이블의 모든 이미지 변경명을 조회
		List<String> dbList = service.selectImageList();
		
		// 2. Server에 저장된 모든 이미 파일 조회
...
//				=file.toString().substring(file.toString().lastIndexOf("\\")+1);
				// /파일명.jsp 만 나옴 +1 추가해서 / 절삭 
				
//				방법 2)
				=file.getName();
				
				// dbList에서 fileName과 일치하는 파일 명이 없다면
				// == 서버에는 있는데 DB에 없는 파일
