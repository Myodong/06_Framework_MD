...
//	@RequestMapping(value="/member/login",method = RequestMethod.POST)
	...
	// - 메서드 매개변수에 전달받은 파라미터를 주입하는 어노테이션
	 ...
	 // [속성]
	   ...
	   // -> required = false 인 경우 전달된 파라미터가 없으면 null
...
//	@RequestMapping(value="/member/login",method = RequestMethod.POST)
	...
	// -> Spring Container에서 Argument(매개변수) Resolver(해결사) 제공해서
	// 	  유연하게 처리함
	...
	public String login(/* @ModelAttribute */ Member inputMember, Model model,RedirectAttributes ra,
		...
		// 로그인 성공 시 loginMember를 세션에 추가
			...
			// 기본 : HttpServleRequest req;
			// 		: req.getHeader("referer");
			
			// 새로운방법 : @RequestHeader(value="referer") String referer
			//				path = referer;
...
//			model.addAttribute("message","아이디 또는 비밀번호가 일치하지 않습니다.");
	...
	public String signUp(Member inputMember /* 커맨드 객체*/,
		...
		// Spring은
		// 1) 같은 name의 속성을 가진 input 태그의 값을
		...
		// 주소가 작성되지 않은 경우 ==> null 이여야함
		if (inputMember.getMemberAddress().equals(",,")) { // 주소가 작성되지 않은경우
			inputMember.setMemberAddress(null);
			
			// 주소가 작성된 경우  ==> 주소,,주소,,주소,, 형식으로 변경
		} else { 
...
//주석 동작안함	@ExceptionHandler(Exception.class)
